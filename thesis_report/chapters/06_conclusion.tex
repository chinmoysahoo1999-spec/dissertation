...
layer rather than from a cleverer feature. Third, the result is model-dependent: a second model
of comparable size, in an extended run, favoured one of the proposed configurations instead.
With only one fully verified model, we deliberately do not generalise.
...
\textbf{One fully measured model.} Only one model has a complete, file-backed set of
configuration and baseline measurements. The second large model is only partly verifiable,
and the remaining large models have generated data but no reported detection results. The
headline comparison therefore rests on a single model, and the cross-model claim is
...
\textbf{Pending archival.} The extended run on the second model is recorded in a comparison
table but its consolidated measurement file is not in the current snapshot, so its numbers are
reported as indicative rather than verified.

\section{Future work}

Several concrete next steps follow from these limitations.

The most immediate is to \emph{complete the sweep}: run the full configuration and baseline
comparison on the remaining seven-billion models, regenerate the gated model's data at full
size, and archive every consolidated measurement file so that all numbers become file-backed.
This turns the present single-model result into a genuine cross-model comparison.
