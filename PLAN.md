...
- **2026-05-30** — Confirmed: **`<tag>_dataset_full.json` schema (text + label + canonical + MIND scalars) is sufficient for extending to any future baseline** that takes (text, label) as input. The schema is method-agnostic; each method reloads the LLM and computes its own features. Multi-sample baselines (SelfCheckGPT, Semantic Entropy) need K samples per test prompt, generated on-the-fly at eval time, not stored. Data-gen doesn't need to be re-run for new methods.
- **2026-05-30 (final)** — **Shared eval-dataset downloader added at `Code/eval_datasets/00_download_eval_datasets.ipynb`**. Run once per HF account to produce `eval_*.parquet` files. All 20 analysis notebooks (`all_variants.ipynb`, `02_all_variants.ipynb`, `03_baselines_sota.ipynb` across the fleet) now check for local parquet files via `_find_local_parquet(label)` before falling back to HuggingFace download. Candidate paths: current dir, `./eval_datasets/`, `/kaggle/input/dissertation-eval-datasets/`, `/kaggle/input/eval-datasets/`, `/kaggle/working/`, `/content/drive/MyDrive/eval_datasets/`, `/content/`. Saves ~5 min HF resolution per dataset × 7 datasets × 12 runs = ~7 h of cumulative wall-clock across the fleet.
- **2026-05-30 (final, late)** — **10 eval datasets** (added NQ-Open, HotpotQA distractor, PopQA — the three most-cited factual-QA benchmarks in 2024-25 hallucination papers). All 27 analysis notebooks evaluate on all 10. HaloScope batch size corrected to 512 (matches paper exactly).
- **2026-05-30 (final, late)** — **6 SOTA baselines** in every `03_baselines_sota.ipynb`. Added:
  * **Lookback Lens** (Chuang et al., EMNLP 2024) — supervised, attention-based. Adds the attention paradigm.
  * **Semantic Entropy** (Farquhar et al., Nature 2024) — unsupervised, NLI-clustering. Adds the NLI paradigm.
  Final paradigm coverage: simplest supervised probe (SAPLMA), best unsupervised (HaloScope), best sampling+geometric (EigenScore), the senior's work (HalluShift), attention-based supervised (Lookback Lens), NLI-clustering sampling (Semantic Entropy). Four paradigms × six methods.
- **2026-05-30 (final, late)** — **Auto-download cell** appended to every 01/02/03 + `all_variants.ipynb`. Colab triggers browser download via `google.colab.files.download`; Kaggle prints persistence note; local Jupyter lists working dir. Downloads small files only (results JSON + .pth checkpoints); excludes the 100+ MB `dataset_with_features.json` which is regeneratable by re-running Stage 3.
- **2026-05-30** — Large-model rollout (Qwen-3B Colab + Llama-2-7B Kaggle). Both `all_variants.ipynb` and `baselines_sota.ipynb` re-emitted via `outputs/build_all_variants.py` and `outputs/parameterize_baselines_sota.py`. HalluShift Wasserstein replaced with vectorised cumulative-diff form (5-10× faster, mathematically equivalent). EigenScore batched generation preserved.
- **2026-05-30** — All large models bumped to 1000 samples/class; added Mistral-7B-v0.1 + OPT-6.7B to the fleet. 3-notebook split (`01_data_generation` + `02_all_variants` + `03_baselines_sota`) applied to every large model for parallel-account execution. Downstream subsample cap = 0.2.
- **2026-05-30** — `Code/eval_datasets/00_download_eval_datasets.ipynb` introduced. Downloads 10 eval datasets (added NQ-Open, HotpotQA, PopQA on top of original 7) as `eval_*.parquet` files. All 02/03 notebooks now check 7 candidate local-parquet paths before falling back to HF Hub.
- **2026-05-31** — Lookback Lens + Semantic Entropy added to source `Code/project_smoke_gpt2/baselines_sota.ipynb` and propagated to all 6 large-model 03 notebooks via `outputs/patch_baselines_add2.py`.
- **2026-06-01** — **Semantic Entropy REMOVED** from the source notebook and all 6 large-model 03 copies. Reason: SE requires an extra ~180M-param NLI model plus K-sample bidirectional clustering, adding ~3-4 hr per 7B model and breaking the 2-3 h per-Colab-Kaggle-session budget. Final baseline set frozen at **5 SOTA**: SAPLMA, HaloScope, HalluShift, EigenScore (INSIDE), LookbackLens. Removal script + backup: `outputs/remove_semantic_entropy.py`, `Code/project_smoke_gpt2/baselines_sota.ipynb.bak_se_remove`. Eval-dataset pre-download time-savings quantified at the same time: **~111 min saved (1.9 h) across 6 models in single-account sequential** mode, **~60 min saved (1.0 h) in 6-account parallel** mode, versus repeating HF Hub resolution in every 02 + 03 run.
- **2026-06-01 (part 2)** — **LookbackLens REMOVED.** With Semantic Entropy already gone, the 4 remaining baselines (SAPLMA / HaloScope / HalluShift / EigenScore) cover every major paradigm. LL added a row but no new paradigm (HalluShift already covers attention statistics). Final 4-baseline set is now FROZEN. Removal script + backup: `outputs/remove_lookback_lens.py`, `Code/project_smoke_gpt2/baselines_sota.ipynb.bak_ll_remove`. All 7 affected notebooks verified LookbackLens-free and AST-clean.
