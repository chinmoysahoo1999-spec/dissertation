...
### 2026-06-01 session (part 2) — LookbackLens removed; FINAL 4-baseline set locked in
1. **LookbackLens baseline REMOVED** from `Code/project_smoke_gpt2/baselines_sota.ipynb` and all 6 large-model `03_baselines_sota.ipynb`. Patch script: `outputs/remove_lookback_lens.py`. Backup of pre-removal source: `Code/project_smoke_gpt2/baselines_sota.ipynb.bak_ll_remove`.
2. **Rationale**: with Semantic Entropy already gone earlier today, the remaining 4 baselines cover every major paradigm — simple supervised probe (SAPLMA), unsupervised spectral (HaloScope), supervised multi-feature (HalluShift), and sampling-based geometric (EigenScore). LookbackLens added one row but no new paradigm: HalluShift already exploits attention statistics via its 5 Wasserstein + 5 cosine attention features. Dropping LL keeps the dissertation's "we beat the senior on her own benchmark" narrative clean and removes one more moving part from the final 7B runs.
3. **FINAL baseline set (frozen): 4 SOTA methods** — SAPLMA · HaloScope · HalluShift · EigenScore (INSIDE). No further additions planned.
4. **Removal verified**: every one of the 7 affected notebooks now has `LookbackLens=0`, `ll_clf=0`, `extract_lookback_features=0`; the 4 retained baselines all still present (EigenScore=22, SAPLMA=44, HaloScope=23, HalluShift=31 occurrences); all 7 notebooks parse cleanly (magic-stripped AST).
5. **Cell-8 layout preserved**: instead of deleting the cell (which would shift downstream cell indices and confuse anyone reading the notebook), the LL training block was replaced with a 5-line comment placeholder noting the removal. The 13-cell layout is now consistent across all 7 baselines_sota notebooks.
